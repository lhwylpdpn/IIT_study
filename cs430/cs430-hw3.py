...
class Solution(object):
    ...
    def __init__(self,cell):
        ...
        print(self.recursively_withoutmem(cell,i=0,j=0))
    ...
    def recursively_withoutmem(self,cell,i,j):
        
      
            if i==len(cell)-1 and j==len(cell[0])-1:

                return cell[len(cell)-1][len(cell[0])-1]
            if i>=len(cell)-1: #move right
               
                return cell[i][j]+self.recursively_withoutmem(cell,i,j+1)
            if j>=len(cell[0])-1:# move down
                return cell[i][j]+self.recursively_withoutmem(cell,i+1,j)
            
            return max(cell[i][j]+self.recursively_withoutmem(cell,i,j+1),cell[i][j]+self.recursively_withoutmem(cell,i+1,j))
    ...
    def iteratively_withmem(self,cell):
        
      

            for i in range(0,len(cell)):

                for j in range(0,len(cell[0])):
                    ...
                    if i==0:
                        ...
                        cell[i][j]=max(cell[i][j-1]+cell[i][j],cell[i-1][j]+cell[i][j])

            return cell[len(cell)-1][len(cell[0])-1]



def recur_nomem(cell,i,j):
        
      
            if i==len(cell)-1 and j==len(cell[0])-1:
                return cell[len(cell)-1][len(cell[0])-1]
            if i>=len(cell)-1: # only can move right
                return cell[i][j]+recur_nomem(cell,i,j+1)
            if j>=len(cell[0])-1:# only can move down
                return cell[i][j]+recur_nomem(cell,i+1,j)
            
            return max(cell[i][j]+recur_nomem(cell,i,j+1),cell[i][j]+recur_nomem(cell,i+1,j))
           
def main(cell):

    return(recur_nomem(cell,i=0,j=0))
...
#   [4,2,1]
# ]
    print(iteratively_withmem(grid))